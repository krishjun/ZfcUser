...
class User extends EventProvider implements ServiceManagerAwareInterface
{
    ...
    public function subscribe(Product $product) {
        $insert = array();
        $insert['user_id'] = $this->getAuthService()->getIdentity()->getId();
        $insert['product_id'] = $product->getProductId();
        $insert['active'] = 1;
        $insert['campaign_id'] = 1;
        $now = new \DateTime();
        $insert['date_created'] = $now->format(\DateTime::ISO8601); 
    
        $subscriptionTb = $this->getServiceManager()->get('subscriptionTb');
        $ret = $subscriptionTb->insert($insert);
        return $ret;
    }
    ...
    public function unsubscribe(Product $product) {
        $where = array();
        $where['user_id'] = $this->getAuthService()->getIdentity()->getId();
        $where['product_id'] = $product->getProductId();
 
    
        $subscriptionTb = $this->getServiceManager()->get('subscriptionTb');
        $ret = $subscriptionTb->delete($where);
        return $ret;
    }
    ...
    public function getSubscriptions() {
        ...
        $subscriptionTb = $this->getServiceManager()->get('subscriptionTb');
        $rows =  $subscriptionTb->select(array('user_id'=>$userId));
        $ret = array();
        $rows = $rows->toArray();
       // var_dump($rows);exit;
        foreach($rows as $row) {
